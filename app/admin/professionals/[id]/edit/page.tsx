  ...
  useEffect(() => {
    async function loadProfessional() {
      try {
        ...
        if (!res.ok) {
          setError(data.error || 'Failed to load professional.');
          setLoading(false);
          return;
        }
        ...
        setForm({
          ...
          lastName: p.lastName ?? '',
          category: p.category ?? 'ATTORNEYS',
          phone: p.phone ?? '',
          email: p.email ?? '',
        });
        setSelectedPracticeAreas(
          p.practiceAreas ? String(p.practiceAreas).split(',').map((s: string) => s.trim()).filter(Boolean) : []
        );
        setBiography(p.biography ?? '');
        setEducation((p.education && p.education.raw) || '');
        setAdmittedToPractice(
          Array.isArray(p.admittedToPractice)
            ? p.admittedToPractice.join('\n')
            : ''
        );
        // memberships were folded into biography; keep as empty here
        setLoading(false);
      } catch (err) {
        console.error(err);
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4 py-8">
      <div className="max-w-3xl mx-auto bg-white rounded-xl shadow p-6">
        <h1 className="text-xl font-semibold mb-4">Edit Professional</h1>
        {error && (
